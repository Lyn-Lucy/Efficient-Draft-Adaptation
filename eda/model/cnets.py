            ...
            if key in current_state_dict:
                ...
                if current_state_dict[key].shape == value.shape:
                    ...
                    print(f"Warning: Shape mismatch for key {key}, skipping. "
                          f"Current: {current_state_dict[key].shape}, Checkpoint: {value.shape}")
            else:
                missing_keys.append(key)
        
        self.load_state_dict(current_state_dict, strict=False)
        print(f"Loaded {len(loaded_keys)} keys from {pretrained_path}")
        if missing_keys and len(missing_keys) < 10:  # 只打印少量缺失的键
            print(f"Missing keys in current model: {missing_keys[:10]}")
        return loaded_keys
