...
parser.add_argument('--pretrained', type=str, default=None, help='Path to pretrained checkpoint for initialization')
...
train_config = {
    ...
    "residual": "true,norm",
    "max_len": 2048,
    "config_path": "qwen2_7B_config_s1p1_baseline.json",
    "b1": 0.9,
    "b2": 0.95,
    "grad_clip": 0.5,
}
...
from eda.model.cnets import Model
...
if rank == 0:
    ...
    train_script_save_path = os.path.join(args.cpdir, "main_deepspeed.py")
    shutil.copy(current_file, train_script_save_path)
    print(f"  - Saved training script to: main_deepspeed.py")
    
    # 保存模型文件
    model_file = os.path.join(os.path.dirname(os.path.dirname(current_file)), "model", "cnets.py")
    if os.path.exists(model_file):
        model_save_path = os.path.join(args.cpdir, "cnets.py")
        shutil.copy(model_file, model_save_path)
        print(f"  - Saved model file to: cnets.py")

    # 保存配置文件
    config_file = os.path.join(os.path.dirname(os.path.dirname(current_file)), "config", "config.json")
    if os.path.exists(config_file):
        config_save_path = os.path.join(args.cpdir, "config.json")
        shutil.copy(config_file, config_save_path)
        print(f"  - Saved config file to: config.json")
...
for epoch in range(num_epochs):
    ...
    for batch_idx, data in enumerate(pbar):
        ...
        predict = model_engine(data["hidden_states"].to(rank).bfloat16(), input_ids=data["input_ids"].to(rank),
    ...
    # 保存 config.json
    if rank == 0:
        config_src = train_config["config_path"]
